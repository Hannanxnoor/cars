...
public class MemberService {
    ...
    public List<MemberResponse> getMembers(boolean includeAll) {
        ...
        for(dat3.cars.entity.Member member: members){
            MemberResponse mr = new MemberResponse(member, includeAll);
            response.add(mr);
        }
        //List<MemberResponse> response = members().stream().map().
        return response;
    }
   ...
   // http://localhost:8888/api/members
    ...
    //Lamda her explain what it isss
    ...
    public ResponseEntity<Boolean> editMember(MemberRequest body, String username) {
        ...
        member.setFirst_name(body.getFirstName());
        member.setLast_name(body.getLastName());
